...
`
  ...
  main.variable(observer("buildvis")).define("buildvis", ["md","container","dc","dateDimension","d3","magnitudeDimension","groupMagnitude","depthDimension","groupDepth","dataset","timeDimension","groupTime"], function(md,container,dc,dateDimension,d3,magnitudeDimension,groupMagnitude,depthDimension,groupDepth,dataset,timeDimension,groupTime)
{
  let view = md`${container()}`
  
  // tabela
  let dcTable = dc.dataTable(view.querySelector("#dc-table-graph"))
  ...
  // gráfico de barras sobre a magnitude
  let dcBarChartMagnitude = dc.barChart(view.querySelector("#magnitude-chart"))
  ...
  dcBarChartMagnitude
            ...
            .group(groupMagnitude)
            .x(xScale)
            .gap(56)
            .elasticY(true)
  
  dcBarChartMagnitude.render()
  
  // gráfico de barras sobre a profundidade
  let dcBarChartDepth = dc.barChart(view.querySelector("#depth-chart"))
...
facts.dimension(d => d.magnitude)
  ...
  main.variable(observer("groupMagnitude")).define("groupMagnitude", ["magnitudeDimension"], function(magnitudeDimension){return(
...
facts.dimension(d => d.depth)
  ...
  main.variable(observer("groupDepth")).define("groupDepth", ["depthDimension"], function(depthDimension){return(
...
facts.dimension(d => d3.timeHour(d.dtg))
  ...
  main.variable(observer("groupTime")).define("groupTime", ["timeDimension"], function(timeDimension){return(
timeDimension.group()
)});
