...
`
  ...
  main.variable(observer("buildvis")).define("buildvis", ["md","container","dc","d3","tipoDimension","tipoGrupo","colorScale","dataset"], function(md,container,dc,d3,tipoDimension,tipoGrupo,colorScale,dataset)
{
  let view = md`${container()}`
  
  // gráfico de barras por tiṕo
  let barChartTipo = dc.barChart(view.querySelector('#type-chart'))
  ...
  // gráfico de linhas por tempo
  let composeChartLine = dc.compositeChart(view.querySelector('#day-chart'))
  
  composeChartLine
    .width(480)
    .height(200)
    .x(d3.scaleTime().domain(d3.extent(dataset, d => d3.timeDay(d.data))))
    .xUnits(d3.timeDays)
...
{
  buildvis;
  let mapInstance = L.map('mapid').setView([41.899510, -87.706908], 5)
  L.tileLayer('http://{s}.tile.osm.org/{z}/{x}/{y}.png', {
    attribution: '&copy; <a href="http://www.openstreetmap.org/">OpenStreetMap</a> constributors',
    maxZoom: 17
  }).addTo(mapInstance)
  return mapInstance
}
  ...
  main.variable(observer("circles")).define("circles", ["circlesLayer","dataset","L","colorScale"], function(circlesLayer,dataset,L,colorScale)
{
  ...
  dataset.forEach( function(d) {
    ...
    circle.bindPopup('Crime Type: ' + d.tipo + '<br>' + 'Time: ' + d.data)
...
d3.csv("https://gist.githubusercontent.com/emanueles/13dfe2c1d43e11b5207bb4e6125d71bf/raw/41d14f8fbd9d6cd9e8ad074f8417cff1329ab339/chicago_crimes_sept_2019.csv").then(function(data){
  let parseData = d3.utcParse("%m/%d/%Y")
  
  data.forEach(function(d){
    d.data = parseData(d.Date.substr(0,10))
...
crossfilter(dataset)
  ...
  main.variable(observer("dataDimension")).define("dataDimension", ["facts"], function(facts){return(
facts.dimension(d => d.data)
)});
  main.variable(observer("tipoDimension")).define("tipoDimension", ["facts"], function(facts){return(
facts.dimension(d => d.tipo)
)});
  main.variable(observer("dataGroup")).define("dataGroup", ["dataDimension"], function(dataDimension){return(
dataDimension.group()
)});
  main.variable(observer("tipoGrupo")).define("tipoGrupo", ["tipoDimension"], function(tipoDimension){return(
tipoDimension.group()
)});
